...
def detect_camera_angle(lm_sequence):
    ...
    for lm in lm_sequence:
        if lm is None:
            continue
        try:
            l_shldr = lm[LM.LEFT_SHOULDER.value]
            r_shldr = lm[LM.RIGHT_SHOULDER.value]
...
def landmarks_visible(lm, required, threshold):
    ...
    for lm_enum in required:
        if lm[lm_enum.value].visibility < threshold:
            failed.append(lm_enum.name)
    ...
    with mp_pose.Pose(**POSE_CFG) as pose:
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                fd.mean_landmark_visibility = float(np.mean([lm[l.value].visibility for l in REQUIRED_LANDMARKS]))
